...
* A key beginning with a single `~` is reserved throughout the document, and one the reader does not recognise is now an error rather than a name. The escape rule already doubled the prefix on any name of your own, so nothing this package writes is affected; a foreign document carrying such a key was read as data before and is refused now. That is what lets a later tag land without every reader built before it silently returning a wrong value.

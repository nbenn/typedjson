...
* A key beginning with a single `~` is reserved throughout the document, and one the reader cannot use as a name is now an error. Since a name is a string and JSON keys are strings, the only name JSON cannot carry is `NA_character_`, so `~zNA_character_` is the one string tag a key may hold and the rest are refused there — which also removes an ambiguity, as `{"~zInf":1}` and `{"~~zInf":1}` both rebuilt the name `"~zInf"` before. The escape rule already doubled the prefix on any name of your own, so nothing this package writes is affected; a foreign document carrying such a key was read as data before and is refused now. That is what lets a later tag land without every reader built before it silently returning a wrong value.

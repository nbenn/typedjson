...
On read, a `~` string is a tag when what follows the prefix is a discriminator the format has reserved. The reserved set is `z`, which spells every tag above, and `:`, which is held for a spelling not yet in use; a reserved string this reader does not know is refused rather than rebuilt as a plausible value. Every other tilde-leading string is a literal, so `~/data` stays a path and "a string is always a string" holds for all of foreign JSON that the format has no claim on.
...
Reserving an alphabet rather than the bare prefix is where the two positions part company, and the reason is that a tilde-leading string is ordinary in foreign JSON while a tilde-leading key is not. Reserving late does not work either way, since a discriminator only earns a refusal from readers that already carry it, which is why `:` lands in the release that names it rather than the one that spends it.
...
Bare environments, closures, language objects and external pointers stay out. The principled line is that an environment is rescuable exactly when something else can supply its shape; for an instance that is the generator, for a generator its own name, and for a bare environment nothing can. What the writer refuses, it refuses loudly, naming the path it stopped at.

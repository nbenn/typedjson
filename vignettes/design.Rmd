...
Everything above records a class by the name that finds it again wherever one does, and a reference resolves against the reader's session rather than the writer's. A writer may want the other answer, because the document is an archive rather than a wire format, and the `embed` flag on both writers is where that is asked for.

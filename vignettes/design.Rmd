...
Attributes ride the ordinary rule, which is what still refuses a formula, since `y ~ x` carries the environment it was created in. A `{` block parsed with `keep.source = TRUE` is refused for the same reason one step further down, where its `srcref` reaches a `srcfile` environment.

...
Most classes need nothing from this page. An S3, S4 or S7 object is a base type plus attributes, so `json_write()` records it without help and the value that comes back is the value that went in. That rule stops at values which are handles rather than data. Closures, language objects and external pointers all name something that exists only inside the running session, and none of them survives being written down. An environment is recorded — by name where a name finds it again, by its contents otherwise — but only as far as what it binds, so one holding any of the above is refused with it.
...
## A finalizer with nothing to release

There is a second reason to take an `R6` class off the default path, and it is easy to miss because it costs nothing until it costs everything. The `~r6` reviver allocates through `$new()` on a rebuilt generator, so `R6` registers the class's `finalize` on the object that comes back exactly as it does on one the constructor produced. The revived object acquired nothing, and its finalizer runs anyway once the garbage collector reaches it.
...
Session <- R6::R6Class("Session",
  ...
  private = list(finalize = function() released <<- c(released, self$id))
)

released <- character()

live <- Session$new("s1")
revived <- json_read_str(json_write_str(live))
...
Cleanup ran for a session that was never opened. A `finalize` written defensively — one that checks the field it releases and returns early when it is `NULL` — no-ops here, which is why this is a hazard rather than a present bug. One that closes, kills or unlinks unconditionally acts on a handle the revived object never held, and the damage lands on whatever else owns that name.

The pair on this page is the answer for a class it reaches. A `json_state()` method takes the class off the `~r6` path, and a `json_revive()` method that rebuilds through the constructor acquires the resource the finalizer will later release, so registration and ownership line up again — the same argument as `ChunkReader` above, made from the other end.
